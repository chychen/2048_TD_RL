...
class board
{
  ...
  public:
	...
	inline int move(const int &opcode)
	...
	inline void add_random_tile()
	{ // add a new random 2-tile or 4-tile
		int space[16], num = 0;
		for (int i = 0; i < 16; i++)
			if (at(i) == 0)
			{
...
class state
{
  public:
	...
	friend std::ostream &operator<<(std::ostream &out, const state &st)
	{
		...
		if (st.is_valid())
		{
			std::cout << ", value = " << st.value << std::endl
					  << st.after;
		}
		else
		{
  ...
  public:
	...
	static void update_tuple_values(std::vector<experience> eb, float learning_rate)
	{
		for (int i = eb.size() - 1; i >= 0; i--)
		{
			...
			if (i == eb.size() - 1)
			{ // the last experience!!
				error = 0.0 - sp.evaluate_score();
			}
			else
			{
				int reward = spp.move(get_best_move(spp));
				error = reward + spp.evaluate_score() - sp.evaluate_score();
			}
			//--------------------------------------------------
			for (size_t j = 0; j < feature::list().size(); j++)
				feature::list()[j]->update(sp.get_board(), error * learning_rate);
		}
	}
};
...
int main(int argc, const char *argv[])
{
	...
	// std::string load = "0556623.weight";
	// std::ifstream in;
	...
	// std::string save = "0556623.weight";
	// std::ofstream out;
